                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBWS ought to be enough for anybody." - Bill Gates, 2017
    ui->zBWSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBWSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBWS->setEnabled(false);
        ui->pushButtonMintzBWS->setToolTip(tr("zBWS is currently disabled due to maintenance."));

        ui->pushButtonSpendzBWS->setEnabled(false);
        ui->pushButtonSpendzBWS->setToolTip(tr("zBWS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBWSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBWS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBWS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBWS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBWS in ") + 
...
void PrivacyDialog::on_pushButtonSpendzBWS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBWS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBWS
        sendzBWS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBWS
    sendzBWS();
}

void PrivacyDialog::on_pushButtonZBWSControl_clicked()
{
    ZBWSControlDialog* zBWSControl = new ZBWSControlDialog(this);
    zBWSControl->setModel(walletModel);
    zBWSControl->exec();
}

void PrivacyDialog::setZBWSControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBWSSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBWS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BWS Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBWSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBWS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BWS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBWS</b>";
    ...
    // use mints from zBWS selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBWSControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBWSControlDialog::GetSelectedMints();
    }
    ...
    // Clear zBWS selector in case it was used
    ZBWSControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBWS Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BWS, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBWS Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBWS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBWS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBWS "));
    ui->labelzBWSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBWSSyncStatus->setVisible(fShow);
}
