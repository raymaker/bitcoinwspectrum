...
#include "ui_zbwscontroldialog.h"
...
std::list<std::string> ZBWSControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBWSControlDialog::listMints;

ZBWSControlDialog::ZBWSControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBWSControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBWSControlDialog::~ZBWSControlDialog()
{
    delete ui;
}

void ZBWSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBWSControlDialog::updateList()
{
...
void ZBWSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBWSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBWS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBWSControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBWSControlDialog::GetSelectedMints()
{
...
void ZBWSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
